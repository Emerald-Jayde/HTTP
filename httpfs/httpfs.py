...
def handle_client_connection(client_socket, client_address):
    ...
    try:
        ...
        if invalid_path_vals in path:
            client_socket.sendall(protocol_version + " 401 Unauthorized" + 3 * nonlocal CRLF)
        else:
            if verb is "GET":
                if not path.exists():
                    client_socket.sendall(protocol_version + " 400 Bad Request" + 3 * nonlocal CRLF)
                if path.is_file():
                    try:
                        file = open(nonlocal path_to_dir + path.__str__(), "rb")

                        resp_line = protocol_version + " 200 OK" + nonlocal CRLF
                        headers = "Content-Length:" + os.path.getsize(path.__str__()) + nonlocal CRLF
                        msg_body = file.readall() + CRLF
                        client_socket.sendall(resp_line + headers + msg_body)

                        print("", end="\n")
                    except OSError as e:
                        resp_line = protocol_version + " 500 Internal Server Error" + nonlocal CRLF
                        headers = nonlocal CRLF
                        msg_body = "File could not be opened/read from." + nonlocal CRLF
                        client_socket.sendall(resp_line + headers + msg_body)
                ...
                try:
                    ...
                    resp_line = protocol_version + " 500 Internal Server Error" + nonlocal CRLF
                    headers = nonlocal CRLF
                    msg_body = "File could not be opened/written to." + nonlocal CRLF
                    client_socket.sendall(resp_line + headers + msg_body)
                ...
                resp_line = protocol_version + " 503 Service Unavilable" + nonlocal CRLF
                headers = nonlocal CRLF
                msg_body = nonlocal CRLF
                client_socket.sendall(resp_line + headers + msg_body)
